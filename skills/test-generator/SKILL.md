...
description: >
  ...
  使用タイミング: (1)「変更分のテストを書いて」「このブランチにテストを追加して」
  「カバレッジを上げて」などの依頼時、(2) 機能実装後に十分なテスト網羅を確認したい時、
  (3) PR 前に変更コードがテストされているか検証したい時。
  Vitest と Cargo プロジェクトをサポート。
  `/test-generator` や「テストを生成して」「変更をテストして」「差分をカバーして」などで呼び出す。
---
