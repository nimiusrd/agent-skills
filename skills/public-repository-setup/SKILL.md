---
name: public-repository-setup
license: MIT
description: 公開リポジトリをGitHubに新規作成するとき、小さく始められるようにルールセット・Immutable Releases・Issue／PRの初期設定を整える。「公開リポジトリを作って初期設定して」「新しい公開リポジトリのGitHub設定を整えて」などに使用する。作成直後の設定や中断した初期設定の再開も対象とし、既存の公開リポジトリ全体の設定監査、通常のIssue対応・PR作成・リリース発行には使用しない。
---

# Public Repository Setup

公開リポジトリを新しく作る際に、最小限の設定で開発を始められるようにする。ブランチ保護とリリースの不変性を基本にし、レビュー条件・必須チェック・テンプレート・文書・自動化は必要になった時点で追加する。すべてを揃えることを開始条件にしない。
...
- ユーザー指定とリポジトリ固有の方針を優先する。個別項目だけの依頼を全設定の変更に広げず、既存の公開リポジトリの一括移行・監査には拡張しない。
