...
license: MIT
description: 公開リポジトリをGitHubに新規作成するとき、小さく始められるようにルールセット・Immutable Releases・Issue／PRの初期設定を整える。「公開リポジトリを作って初期設定して」「新しいOSSのGitHub設定を整えて」などに使用する。作成直後の設定や中断した初期設定の再開も対象とし、既存OSS全体の設定監査、通常のIssue対応・PR作成・リリース発行には使用しない。
---
